...
def ng_app(name, project_name = None, srcs=[], deps = [], test_deps = [], e2e_deps = [], **kwargs):
    """
    ...
    #     "e2e",
    #     size = "large",
    #     srcs = srcs + deps + e2e_deps + NG_COMMON_DEPS + E2E_DEPS + E2E_CONFIG,
    #     args = [
    #       "--no-webdriver-update",
    #       "--port=0",
    #     ],
    ...
    _architect_test(
        project_name,
        "lint",
        srcs = srcs + test_srcs  + deps + test_deps + NG_COMMON_DEPS + LINT_DEPS + LINT_CONFIG + ([":lint-config"] if native.package_name() != '' else []),
        tags = tags + ["lint"],
        **kwargs
    )
